...
#[get("/")]
pub async fn admin_blog_entries(
    database: &State<database::MongoDB>,
    user: UserSession,
) -> Result<Template, GenericError> {
    get_blog_entries(
        database,
...
#[post("/blog", data = "<form>")]
pub async fn new_blog_post(
    mut form: Form<Contextual<'_, BlogEditViewContext>>,
    database: &State<database::MongoDB>,
    _user: UserSession,
...
async fn get_blog_entries(
    ...
    user_session: Option<UserSessionData>,
) -> Result<Template, GenericError> {
    ...
    match database
        .fetch_all_published_blogs(!user_session.edit_mode)
        .await
    {
        Ok(blogs) => {
