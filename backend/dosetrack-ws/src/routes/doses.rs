...
use crate::database;
use crate::model::dosetrack::{Dosimeter, User, UserRole, UserStatus, Operator, Dose};
...
#[get("/doses/<company_id>", format = "json")]
 pub async fn get_by_company(
   ...
   let collection = database.collection::<Dose>("doses");
    let mut cursor: Cursor<Dose> = collection.find(doc!{ "doses_by_company": ObjectId::parse_str(&company_id).unwrap() }, None).await.unwrap();
...
#[get("/doses/operator/<operator_id>", format = "json")]
 pub async fn get_by_operator(
    ...
    dbg!(&operator_id);
    let collection = database.collection::<Dose>("doses");
    let mut cursor: Cursor<Dose> = collection.find(doc!{ "operator_id": ObjectId::parse_str(&operator_id).unwrap() }, None)
    .await.unwrap();
    let mut doses: Vec<Dose> = Vec::new();
...
// ver qué significa data= new_dosis
  ...
  if new_dosis._id.is_none() {
    ...
    let result = collection.insert_one(new_dosis.deref(), None).await;
    dbg!(result);
  } 
